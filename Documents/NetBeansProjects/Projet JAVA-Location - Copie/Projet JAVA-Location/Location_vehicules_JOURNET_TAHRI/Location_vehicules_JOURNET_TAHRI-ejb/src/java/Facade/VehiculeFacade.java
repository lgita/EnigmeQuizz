...
public class VehiculeFacade extends AbstractFacade<Vehicule> implements VehiculeFacadeLocal {
    ...
    @Override
    public void CreerVehicule(String num, String marque, String modele, Type type, double prixj, double prixs, Double nbkm) {
    ...
    @Override
    public java.util.Collection<Vehicule> RechercheVehiculeLibre(Statut statut) {
        String txt="SELECT v FROM Vehicule AS v WHERE v.num_vehicule.statut=:st ";
        Query req=getEntityManager().createQuery(txt);
