...
public class ReservationFacade extends AbstractFacade<Reservation> implements ReservationFacadeLocal {
    ...
    public void ModificationDate(Reservation r, Date dated, Date datef) {
        r.setDate_debut_reservation(dated);
        r.setDate_fin_reservation(datef);
        r.getLeVehicule().getLeStatut().setDateDebut(dated);
        r.getLeVehicule().getLeStatut().setDateFin(datef);
    ...
    public Reservation RechercheReservation(String num) {
        ...
        Query req = getEntityManager().createQuery("SELECT r FROM Reservation AS r WHERE r.Num_reservation=:nu ");
    ...
    public Reservation RechercheReservationidClient(String idClient, Date datedeb, Date datefin) {
        ...
        Reservation r = null;
        List<Reservation> result;
        Query req = getEntityManager().createQuery("SELECT r FROM Reservation AS r WHERE r.Lecompte.Leclient.Num_client=:id AND r.Date_debut_reservation=:dd AND r.Date_fin_reservation=:df ");
        req = req.setParameter("id", idClient);
        req = req.setParameter("dd", datedeb);
        req = req.setParameter("df", datefin);
        result = req.getResultList();
        if (result.size() == 1) {
            r = (Reservation) result.get(0);
        }
